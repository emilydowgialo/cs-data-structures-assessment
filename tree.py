...
class Tree(object):
    ...
    def depth_first_search(self, data):
        ...
        # Set the node we are looking for to the root, as a list
        node_wanted = [self.root]

        # Pop from the end, and if the node's data is the data we want, return it
        while node_wanted:
            node = node_wanted.pop()

            if node.data == data:
                return node

            # If the node's data is not what we want, we extend to the node's
            # children
            node_wanted.extend(node.children)
